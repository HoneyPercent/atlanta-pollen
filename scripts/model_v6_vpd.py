"""
Model V6 — V5 + VPD (vapor pressure deficit).

VPD is the strongest new weather variable (r=0.344, stronger than temp_max).
Biology research confirms: pollen release depends on anther wall dehydration,
making VPD the mechanistically correct dryness predictor.

Quick test: take V5's feature set, add VPD, re-evaluate.
"""
...
def load_features():
    ...
    for r in rows:
        ...
        for field in ["gdd_daily", "temp_max", "temp_min", "temp_mean", "precipitation",
                      ...
                      "vpd_max", "solar_radiation", "sunshine_duration"]:
...
def compute_window_stats(row, idx, lookback=5):
    ...
    for d in range(1, lookback + 1):
        ...
        if prev:
            ...
            if prev.get("vpd_max") is not None: vpds.append(prev["vpd_max"])
    return {
        ...
        "vpd_5d_avg": statistics.mean(vpds) if vpds else 0,
...
def extract_features_v6(row, idx, rows_by_doy):
    ...
    features = {
        ...
        "vpd_max": vpd,  # NEW: vapor pressure deficit
        "vpd_5d_avg": window["vpd_5d_avg"],  # NEW: 5-day VPD average
...
def main():
    ...
    print("=" * 70)
    print("MODEL V6 — V5 + VPD (vapor pressure deficit)")
    print("=" * 70)
    ...
    for test_yr in test_years:
        ...
        fn = None
        for r in train:
            result = extract_features_v6(r, idx, doy_lookup)
            if result:
                feats, target = result
    ...
    print(f"\n{'Model':<20} {'MAE':>7} {'RMSE':>7} {'Acc':>7} {'Prec':>7} {'Recall':>7}")
    print("-" * 55)
    print(f"{'V6 (V5+VPD)':<20} {avg_mae:>7.3f} {avg_rmse:>7.3f} {acc:>6.1%} {prec:>6.1%} {rec:>6.1%}")
    print(f"{'V5 (best prev)':<20} {'0.724':>7} {'0.967':>7} {'85.3%':>7} {'86.8%':>7} {'90.3%':>7}")
    print(f"{'GBT (nonlinear)':<20} {'0.720':>7} {'0.970':>7} {'85.4%':>7} {'89.7%':>7} {'86.6%':>7}")
    print(f"{'V1 (original)':<20} {'0.822':>7} {'1.078':>7} {'83.3%':>7} {'86.5%':>7} {'86.9%':>7}")

    if last_model:
        print(f"\nVPD-related feature weights:")
        for fname, weight in last_model.feature_importance():
            if 'vpd' in fname.lower():
        ...
        model_data = {
            ...
            "version": "V6", "description": "V5 + VPD (vapor pressure deficit)",
        }
        with open(OUTPUT_DIR / "weather_model_v6_weights.json", "w") as f:
